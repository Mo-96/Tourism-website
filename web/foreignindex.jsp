        ...
        <%
            ...
            for (localpost post : posts) {
               ...
               <div class="col-lg-3 col-md-6 visit mb-3" data-aos="fade-right">
                    ...
                    <h2><a href="#.html"><%=post.getTitle()%></a></h2>
                    <div class="reviews-star float-left">
                     <%=post.getDescription()%>
                    </div>
                    <form action="#.jsp">
                    <span class="reviews-star float-right">
                        <h5> Contact NO: <% String tel;int userid = post.getUid();
                        ResultSet search = DB.search("SELECT `tel` FROM `locals` WHERE id = '"+userid+"'");
                        if (search.next()) {tel = search.getString(1);out.print(tel);}
                        %>
                        <br>
                       Email:  <% String email;int userid1 = post.getUid();
                        ResultSet search1 = DB.search("SELECT `email` FROM `locals` WHERE id = '"+userid1+"'");
                        if (search1.next()) {email = search1.getString(1);out.print(email);}
                        %>
                        </h5>
                    </span>
                    </form>
